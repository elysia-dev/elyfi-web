...
const calcAccumulatedYield = (
	...
	if (balance.isZero() && mints.length > 0 && burns.length > 0) {
		...
		index = BigNumber.from(lastBurnIndex > lastMintIndex ? lastBurnIndex : lastMintIndex);
	}

	const mintYield = mints
		.reduce((res, cur) => res.add(BigNumber.from(cur.amount).mul(index.sub(BigNumber.from(cur.index)))), constants.Zero)
		.div(RAY)
