import { utils } from 'ethers';

export const ELFIPerDayOnELStakingPool = utils.parseEther('25000');

export const TETHERPerDayOnELFIStakingPool = utils.parseEther('1250');

export const DAIPerDayOnELFIStakingPool = utils.parseEther('183587').div(28);
export const BUSDPerDayOnELFIStakingPool = utils.parseEther('183587').div(28);

export const LPPerDayOnElfiEthPool = utils.parseEther('183587').div(28);
export const LPPerDayOnElfiDaiPool = utils.parseEther('183587').div(28);

export const ELFIPerDayOnLpStakingPool = 7500;
export const DAIPerDayOnElfiDaiPool = 625;
export const ETHPerDayOnElfiEthPool = 0.1376;

export const ELFI_REWARD_PER_POOL = 300000;
export const ETH_REWARD_PER_POOL = 5.507;
export const DAI_REWARD_PER_POOL = 25000;
